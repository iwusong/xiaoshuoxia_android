...
$(document).ready(function () {
    $("div.box03").click(function () {
        onThreadOptionsClick();
    });
    $(".reply_botton_j a").click(function () {
        loadMore();
    });
    $(".thread_details .P_share div a:eq(0)").click(function () {
        praise();
    });
    $(".thread_details .P_share div a:eq(1)").click(function () {
        report();
    });
    $(".thread_details .P_share div a:eq(2)").click(function () {
        share();
    });
    $(".thread_details .P_share div a:eq(3)").click(function () {
        report();
    });
//			   test();
...
/**
 *如果客户端webview on loading不能执行改方法,由android客户端调用(“javascript:onRefresh(JSONdata)”)
 *@param JSONdata 帖子详情数据
...
function onLoadReply(JSON, isAppend) {
    ...
    if (!textIsNull(postlist)) {
        for (var i = isAppend ? 0 : 1; i < postlist.length; i++) {
            ...
            li.attr({
                "pid": postlist[i].pid,
                "authorid": postlistitem.authorid,
                "id": "rednet_anchor_id_" + postlistitem.pid
            });//设置#锚点
            li.find("div.ageImg img")
                .click(function () {
                    userInfo($(this).parents("li").attr("authorid"));
                });
            li.find("div.messContent p:eq(0) a").html(postlistitem.author).click(function () {
                userInfo($(this).parents("li").attr("authorid"));
            });
...
//			li.find("div.messContent p:eq(0) span").html("沙发");
            ...
            li.find("div.P_share a.huifu").click(function () {
                discussUser($(this).parents("li").attr("pid"));
            });
            li.find("div.P_share a.jubao").click(function () {
                reportComment($(this).parents("li").attr("pid"));
            });
...
/*@hidden*/
function threadThumbsClick(url) {
    if (is2GOr3GLoadImgs) {
...
//回复用户评论
/*@hidden*/
function discussUser(pid) {
    Rednet.onDiscussUser(pid);
}

//举报用户评论
/*@hidden*/
function reportComment(pid) {
